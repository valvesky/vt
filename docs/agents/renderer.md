...
I have no render thread. `peak_wait` on window fd, PTY, ctl listen, clients, one job pipe, SIGUSR1. PTY stays in that set on the `hz` hold. Ingest every wake: fill the 64 KiB ring until EAGAIN/EOF, then preparse that unread. Ring full → ingest again with no `peak_wait` until EAGAIN or the present deadline. Present still at most `hz`. SIGUSR1 re-reads `~/.config/omarchy/current/theme/alacritty.toml` into the palette and presents. Missing file keeps `config.h`. Timeout is `-1` while `!redraw`, `0` when `now >= next_present`, else remaining ms (min 1) while dirty and before `hz`. After present: `redraw` clears, `next_present = now + 1s/hz`. Do not wake me when `!redraw`. Peak still wakes on the window fd; do not use Display-wide `XPending` for timeout 0.
