...
Client: `vtctl --help`. I speak JSONL: one object + LF. `$XDG_RUNTIME_DIR/vt/<pid>.sock` else `/tmp/vt-<uid>/<pid>.sock`. `latest.sock` → newest. Windowed and `--live`.

Loop `vtctl read` → `vtctl rg needle` → `vtctl write keys` → `vtctl read`. `write` = my PTY bytes (parser). `read`/`rg` = my live grid. `dump` = full grid (tests). `screenshot` = CPU atlas. Present/lag: stderr (`2>log`). Idle/present: `docs/agents/renderer.md`. Pane drop: `docs/agents/features.md`.
...
| Op | Request | Reply |
|----|---------|-------|
| `read` | `{op:"read"}` or `{op:"read","y":20,"n":8}` | `ok`, `x`, `y`, `cols`, `rows`, `text` (those rows, same trim as `dump_walk`) |
| `rg` | `{op:"rg","data":"..."}` | `ok`, `n`, `text` (`<y>:<row>`, 0-based y). Cap → `"trunc":true` |
| `dump` | `{op:"dump"}` | `ok`, `cols`, `rows`, `text` (`--headless` dump shape) |
...
| `hit` | `{op:"hit"}` | `ok`, `hit` 0/1. `x`,`y` cells if pointer is in this window. `--live` is always 0 |
...
`rg` is `memmem` per row, not regex. One string, rg shape, e.g. `{"ok":true,"n":2,"text":"24:  sel_utf8(char *dst, size_t cap)\n87:  static size_t sel_utf8(...)"}`. No match objects.
...
Besides my child, only ctl `write` and `--headless` file ingest enter my parser.

## Limits

4 clients. Line 8192. `rg` 64 hits / 8192 `text`. No window or Vulkan; `--live` is enough.
