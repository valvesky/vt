...
`src/vt_renderer.c`: file-scope `PeakWindow win` and `Renderer renderer`. Incomplete `Renderer` is in `src/vt.h`. Peak owns my window + event fd. Rend owns GPU/CPU. I do not compile shaders: `build.c` runs `slangc` on `vulkan/vt.slang` (`vertMain` / `fragMain`) → `vulkan/vt.vert.spv`, `vulkan/vt.frag.spv`. CPU path is `vulkan/vt.cpu.c` via `rend_pipeline_create_graphics_c` (slang `-target c` has no separate vert/frag hosts). `./build cpu` skips Vulkan. Rend `AUTO` may fall back to `REND_BACKEND_CPU`. API: `rend.h` and Peak headers, not library `.c`.
