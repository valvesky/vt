...
I have no render thread. `peak_wait` on window fd, ctl listen, clients, one job pipe, SIGUSR1. PTY is in that set only when I am idle (`timeout == -1`) or a frame is due (`timeout == 0`). Positive timeout = `hz` hold: no PTY, no ingest. SIGUSR1 re-reads `~/.config/omarchy/current/theme/alacritty.toml` into the palette and presents. Missing file keeps `config.h`. Timeout is `-1` while `!redraw`, `0` when `now >= next_present`, else remaining ms (min 1) while dirty and before `hz`. After present: `redraw` clears, `next_present = now + 1s/hz`. Do not wake me when `!redraw`. Peak still wakes on the window fd; do not use Display-wide `XPending` for timeout 0.
