...
`tests/badutf.bin` must feed without abort. `--screenshot` must write P6 PPM. If `python3`: `tests/clip` (ctl clipboard + OSC 52 via `vt-live`), `tests/ctl` (`read`, `rg`, `log`, `latest.sock`), `tests/mux` (ctl `split`/`focus`/`panes`/`move`; two-live `adopt`/`give` SCM_RIGHTS; dump/write stay on focus), `tests/kitty_query` (APC `a=q` reply before DA). Mouse pane-drop is manual (`docs/features.md`).
