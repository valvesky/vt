...
static u32 renderer_fill(Term *term, TermScreen *s, u32 ox, u32 oy, u32 cx, u32 cy, int cursor_on, color_packed_t cur_fg, color_packed_t cur_bg, int sel_on, u32 sel0, u32 sel1, VtInstance *inst, u32 n, u32 cap);
static u32 renderer_fill_cp(u32 x, u32 y, codepoint_t cp, color_packed_t fg, color_packed_t bg, VtInstance *inst, u32 n, u32 cap);
static void renderer_flush(u32 n);
...
renderer_get_grid(Renderer *r, u32 *cols, u32 *rows)
{
    ...
    *rows = atlas.cell_height ? r->current_height / atlas.cell_height : 0;
    ...
    static u32
renderer_fill(Term *term, TermScreen *s, u32 ox, u32 oy, u32 cx, u32 cy, int cursor_on,
        color_packed_t cur_fg, color_packed_t cur_bg, int sel_on, u32 sel0, u32 sel1,
        VtInstance *inst, u32 n, u32 cap)
{
    u32 x;
    u32 y;
    u32 cols;
    ...
    static u32
renderer_fill_cp(u32 x, u32 y, codepoint_t cp, color_packed_t fg, color_packed_t bg,
        VtInstance *inst, u32 n, u32 cap)
{
    vt_glyph_id g;
    ...
    inst[n].foreground = (fg & 0xffffff00u)
        ...
        | ((g & VT_GLYPH_COLOR) ? 0x80u : 0)
        | ((fg >> 3) & 1u) | ((fg >> 6) & 2u);
    inst[n].background = (bg & 0xffffff00u) | (g & 0x7fu);
    return n + 1;
}

    static void
renderer_flush(u32 n)
{
    VtPush pc;
    uint32_t bg;
    float r, g, b;
    ...
    if (!renderer.gpu || !renderer.pipeline)
        return;
    if (glyph_atlas_dirty && atlas.atlas && renderer.atlas_tex.handle) {
        size_t bytes;
    ...
    VTDEBUG("present fill %llu begin %llu draw %llu end %llu ns",
            ...
            (unsigned long long)(t_begin - t0),
            (unsigned long long)(t_draw - t_begin),
            (unsigned long long)(t_end - t_draw));
    vt_stage_add(VT_STAGE_BEGIN, t_begin - t0);
        ...
        color_packed_t cur_fg, color_packed_t cur_bg, int sel_on, u32 sel0, u32 sel1)
{
...
#endif
    {
        ...
        n = renderer_fill(term, s, 0, 0, cx, cy, cursor_on, cur_fg, cur_bg, sel_on, sel0, sel1,
            inst, 0, renderer_ninst);
#ifdef DEBUG
        t_fill = peak_get_time();
        VTDEBUG("present fill %llu ns", (unsigned long long)(t_fill - t0));
